package com.example.fourinoneapp.adapter
...
//    }
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, p1: Int): GVH {
        val vh = GVH(LayoutInflater.from(viewGroup.context).inflate(R.layout.item_gallery, viewGroup, false))
    ...
    override fun onBindViewHolder(gvh: GVH, p1: Int) {
        val picture = list[p1]
        Glide.with(gvh.containerView).load(picture.path).into(gvh.containerView.ivImg)

        if (picture.isSelected) {
            gvh.containerView.vSelected.visibility = View.VISIBLE
        } else {
            gvh.containerView.vSelected.visibility = View.GONE
        }
    }
